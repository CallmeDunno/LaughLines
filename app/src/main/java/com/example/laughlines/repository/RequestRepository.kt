...
import com.example.laughlines.model.ChatModel
...
class RequestRepository @Inject constructor(private val fDb: FirebaseFirestore, private val sharedPref: SharedPreferencesManager) {
    ...
    fun acceptRequest(myId: String, yourID: String) {
        CoroutineScope(Job() + Dispatchers.IO).launch {
            val chat = ChatModel(myId, yourID, 0)
    ...
    private suspend fun addToChat(chat: ChatModel) = fDb.collection(Constant.Collection.Chats.name).add(chat).await().id
